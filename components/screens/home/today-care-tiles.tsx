...
function CareTile({ petId, kind, label, icon: Icon }: { petId?: string } & (typeof TILES)[number]) {
  ...
  return (
    <div className="flex flex-col items-center gap-0.5 rounded-xl bg-muted/60 p-3">
...
export function TodayCareTiles({ petId }: { petId?: string }) {
  return (
    <div className="grid grid-cols-3 gap-3">
      {TILES.map((t) => (
        <CareTile key={t.kind} petId={petId} {...t} />
      ))}
    </div>
  )
}
